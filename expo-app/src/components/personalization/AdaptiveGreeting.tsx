...
import { PersonalizedGreeting } from '../../types/domain';
...
export const AdaptiveGreeting = memo<AdaptiveGreetingProps>(({
  greeting,
  fallbackName,
  style,
  unreadCount = 0,
  ...
  const title = greeting?.title ?? `Hi, ${fallbackName}!`;
  const subtitle =
    greeting?.subtitle ?? 'How are you feeling today?';

  return (
    <Animated.View
      key={greeting?.kind ?? 'fallback'}
      entering={
        reduceMotion ? undefined : FadeInDown.duration(Motion.duration.normal)
      }
...
AdaptiveGreeting.displayName = 'AdaptiveGreeting';
